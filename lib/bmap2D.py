...
class BMap2D(Base):
	...
	def count_ones(self,axis='rows'):
		if axis == 'rows' :
			rv = np.zeros(self.nrows,dtype=int)
			for i in xrange(self.nrows):
				#!pos = i * self.ncols
				#!rv[i] = self.bmap[pos: pos + self.ncols].count()
				rv[i] = self.get_row(i).count()
		else :
			rv = np.zeros(self.ncols,dtype=int)
	...
	def __xor__   (self,b):
		...
		return BMap2D( self.nrows, self.ncols, self.bmap ^ bmap )
	def __invert__(self)  :
		return BMap2D( self.nrows, self.ncols, ~ self.bmap )
	...
	def set_row(self,row, val) :
		if not isinstance(val,int) :
			assert val.length() == self.ncols, "the size:%s of the value differ from the number of cols:%s" % (val.length(), self.ncols)
		if row < 0 : row = self.nrows - abs(row)
		assert row >= 0 and row < self.nrows, "row idx out of range : %s" % row
		pos = row * self.ncols
		if isinstance(val, BMap2D) :
	...
	#bm[:,int], bm[:,[1,2,3]] , bm[:, 1:10:2]
	#.. can return the result as-is i.e. cols OR as rows
	def get_cols(self, lst, ret='bmap', ret_as='cols'): #works with numpy array as list
		...
		if ret_as == 'rows' :
			...
			bm = BMap2D(nrows=self.nrows, ncols=lst_len)
			for i, col in enumerate(lst):
				bm.set_col(i, self.get_col(col))
	...
	def set_byidxs(self, idxs, vals):
		...
		if isinstance(idxs,tuple) :
			...
			#they are no longer slices but ranges or ints
			...
			if isinstance(vals,int) :
				...
				return
			#?? if isinstance(vals, (BMap1D,bitarray)) :
			if isinstance(vals,BMap2D) :
